...
const King_detail = () => {
    ...
    const [ch_num, setCh_num] = useState(0);



    useEffect(() => {
        fetch(`http://127.0.0.1:8000/kings/${id}/`)  // Замените URL на ваш эндпоинт Django
            .then(response => response.json())
            .then(data => {
                if (Array.isArray(data) && data.length > 0) {
                    setModels(data[0]);
                    setCh(JSON.parse( data[0].desc.replace(/'/g, '"')))
                    console.log(ch)
                }
            })
